...
| 26 — Governed external-experiment boundary | `external`, `schemas`, `research`, `cli` | `EXTERNAL_EXPERIMENT` `GovernedAction` with a propose → approve → execute → ingest lifecycle for Regime-C sciences (wet-lab/fabrication/instrument/external-compute). The org proposes a typed `ExternalExperimentSpec` (action class, exact proposal, cost/risk envelope) as a default-deny approval request bound by `governed_action_hash`; a human approves under the Goal 19 identity rules; a human runs the action **outside** `siro`; the operator ingests a signed `ExternalResultRecord` bound to the live approval. The `external-oracle` `ExternalOracleAdapter` (Goal 22) scores on the ingested result instead of running code — it touches no execution plane — and promotes only when a live, matching, signed result resolves; an unapproved / expired / revoked / hash-mismatched / unsigned result is logged `REJECTED` and never promotes. Null/failed results are first-class negatives. CLI: `propose-external-experiment`, `list-external-experiments`, `ingest-external-result`, `external-audit`. |
| 27 — Drug and life-science pack | `packs/life_science/`, `life_science`, `research`, `config/tier{0,1}.life_science.yaml` | Capstone two-stage pack combining both new regimes (regime `statistical` for the screen, per-task `external-oracle` override for confirmation). **Screening (Regime B, offline):** a controller-owned surrogate `eval.py` scores a fragment-token molecule for predicted binding affinity against a held-out kinase target, with drug-likeness (ADMET logP window) and synthesizability (cost ceiling) as hard preconditions, so inflating affinity by stacking lipophilic/bulky groups fails outright; promoted under the Goal 24 confidence-bound gate. Surrogate weights/thresholds/target are controller-owned (`SIRO_HIDDEN_PATH`); the candidate edits only `molecule.txt`. **Confirmation (Regime C, governed):** a screened candidate is proposed (never agent-authorized) as a Goal 26 external wet-lab assay; promotion to *confirmed* requires an ingested, signed result bound to a live human approval — never an in-silico score. `propose_confirmation`/`ConfirmationNotEarned` (`src/siro/life_science.py`) enforce screening-before-confirmation so costly, irreversible assays stay few and high-value; the execution plane runs no synthesis or assay and holds no lab credentials. Negative/null screen and assay results are first-class. `load_research_task` honors a per-task `evaluator_regime` override so one pack carries both stages; the dispatching `LifeScienceAdapter` routes each task. Dual-use posture stated in the pack briefs, references, and role prompts. |
